 ...
 * 标准乐高积木颗粒
 */
const standardButton: FillMosaicRect = (ctx, rgba, x, y, w, h) => {
  // 浮雕背景色
 ...
 * 半球型颗粒
 */
const sphericalButton: FillMosaicRect = (ctx, rgba, x, y, w, h) => {
  ...
  radgrad.addColorStop(1, `rgb(${rgba.map((color) => color * 1.6).join(",")})`);
 ...
 * 平面颗粒
 */
const flatButton: FillMosaicRect = (ctx, rgba, x, y, w, h) => {
  const circle = new Path2D();
  circle.arc(x + w / 2, y + h / 2, w / 3, 0, 2 * Math.PI);
  ctx.fill(circle);
};
